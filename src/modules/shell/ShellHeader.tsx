  ...
  return (
    <div className="flex-shrink-0 border-b border-gray-700 bg-gray-800 px-4 py-2">
      <div className="flex items-center justify-between gap-3">
        {/* min-w-0 lets the status texts shrink (ellipsis) so the action
            buttons keep their one-line labels on narrow portrait screens. */}
        <div className="flex min-w-0 items-center space-x-2">
          <div className={`h-2 w-2 flex-shrink-0 rounded-full ${isConnected ? 'bg-green-500' : 'bg-red-500'}`} />
        ...
        {/* flex-shrink-0 + per-button nowrap: CJK labels squeezed to min-content
            used to wrap one character per line on mobile. */}
        <div className="flex flex-shrink-0 items-center gap-2">
          {showBypassToggle && (
            <button
              ...
              className={`inline-flex h-8 flex-shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md border px-3 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 ${
                bypassEnabled
          ...
          {isConnected && (
            <button
              ...
              className="inline-flex h-8 flex-shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md bg-red-600 px-3 text-xs font-medium text-white transition-colors hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-400/70 focus:ring-offset-2 focus:ring-offset-gray-800"
